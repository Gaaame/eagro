import { Link } from "react-router-dom";

function App() {
  return (
    <div className="font-sans">
      <section id="home" className="relative overflow-hidden bg-[#061810]">
        {/* Background Glow */}
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div
            className="
              absolute
              -left-40
              top-20
              h-96
              w-96
              animate-pulse
              rounded-full
              bg-green-500/10
              blur-3xl
              [animation-duration:4s]
            "
          />

          <div
            className="
              absolute
              -right-40
              bottom-0
              h-[30rem]
              w-[30rem]
              animate-pulse
              rounded-full
              bg-[#8BCF63]/10
              blur-3xl
              [animation-duration:5s]
            "
          />

          <div
            className="
              absolute
              left-1/2
              top-1/2
              h-72
              w-72
              -translate-x-1/2
              -translate-y-1/2
              animate-pulse
              rounded-full
              bg-green-500/5
              blur-3xl
              [animation-duration:6s]
            "
          />
        </div>

        {/* Decorative Dots */}
        <div
          className="
            pointer-events-none
            absolute
            left-[12%]
            top-[20%]
            h-2
            w-2
            animate-ping
            rounded-full
            bg-[#8BCF63]/40
            [animation-duration:3s]
          "
        />

        <div
          className="
            pointer-events-none
            absolute
            right-[15%]
            top-[15%]
            h-1.5
            w-1.5
            animate-pulse
            rounded-full
            bg-[#F5B900]/60
            [animation-duration:2s]
          "
        />

        {/* Main Container */}
        <div className="relative mx-auto max-w-6xl px-6 py-20 sm:py-24 lg:py-28">
          <div className="grid items-center gap-14 lg:grid-cols-2">
            {/* ========================================
                LEFT CONTENT
            ======================================== */}

            <div>
              {/* Badge */}
              <div
                className="
                  animate-entrance-left
                  entrance-delay-100
                  mb-6
                  inline-flex
                  items-center
                  gap-2
                  rounded-full
                  border
                  border-green-500/20
                  bg-green-500/10
                  px-4
                  py-2
                "
              >
                <span className="text-sm text-[#8BCF63]">✦</span>

                <span className="text-xs font-medium text-[#8BCF63]">
                  Farmer's Assistance Program
                </span>
              </div>

              {/* Heading */}
              <h1
                className="
                  animate-entrance-left
                  entrance-delay-200
                  max-w-xl
                  text-5xl
                  font-bold
                  leading-[1.02]
                  tracking-[-0.04em]
                  text-[#F1F5EE]
                  sm:text-6xl
                  lg:text-7xl
                "
              >
                Grow together,
                <br />
                from the field to
                <br />
                the <span className="text-[#8BCF63]">market.</span>
              </h1>

              {/* Description */}
              <p
                className="
                  animate-entrance-left
                  entrance-delay-300
                  mt-7
                  max-w-lg
                  text-sm
                  leading-6
                  text-white/55
                  sm:text-base
                "
              >
                E-Agro is a one-stop shop that helps farmers, suppliers, and
                laborers work toward mutual growth and a better future.
              </p>

              {/* Buttons */}
              <div
                className="
                  animate-entrance-up
                  entrance-delay-400
                  mt-8
                  flex
                  flex-wrap
                  gap-3
                "
              >
                <Link
                  to="/contact"
                  className="
                    group
                    rounded-full
                    bg-[#F5B900]
                    px-6
                    py-3
                    text-sm
                    font-semibold
                    text-[#07150D]
                    shadow-lg
                    shadow-yellow-500/10
                    transition-all
                    duration-300
                    hover:-translate-y-1
                    hover:bg-[#FFC928]
                    hover:shadow-xl
                    hover:shadow-yellow-500/20
                  "
                >
                  Apply for a loan
                  <span
                    className="
                      ml-1
                      inline-block
                      transition-transform
                      duration-300
                      group-hover:translate-x-1
                    "
                  >
                    →
                  </span>
                </Link>

                <a
                  href="#program"
                  className="
                    group
                    rounded-full
                    border
                    border-white/40
                    px-6
                    py-3
                    text-sm
                    font-medium
                    text-white
                    transition-all
                    duration-300
                    hover:-translate-y-1
                    hover:border-[#8BCF63]/50
                    hover:bg-white/10
                  "
                >
                  Learn more
                  <span
                    className="
                      ml-1
                      inline-block
                      transition-transform
                      duration-300
                      group-hover:translate-y-1
                    "
                  >
                    ↓
                  </span>
                </a>
              </div>

              {/* Features */}
              <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3">
                {/* Feature 1 */}
                <div
                  className="
                    animate-entrance-up
                    entrance-delay-500
                    group
                    flex
                    items-center
                    gap-2
                    text-xs
                    text-white/65
                    transition-all
                    duration-300
                    hover:-translate-y-1
                    hover:text-white
                  "
                >
                  <span
                    className="
                      flex
                      h-5
                      w-5
                      items-center
                      justify-center
                      rounded-full
                      border
                      border-[#8BCF63]
                      text-[10px]
                      text-[#8BCF63]
                      transition-all
                      duration-300
                      group-hover:scale-110
                      group-hover:bg-[#8BCF63]
                      group-hover:text-[#061810]
                    "
                  >
                    ✓
                  </span>
                  Low-interest loans
                </div>

                {/* Feature 2 */}
                <div
                  className="
                    animate-entrance-up
                    entrance-delay-600
                    group
                    flex
                    items-center
                    gap-2
                    text-xs
                    text-white/65
                    transition-all
                    duration-300
                    hover:-translate-y-1
                    hover:text-white
                  "
                >
                  <span
                    className="
                      flex
                      h-5
                      w-5
                      items-center
                      justify-center
                      rounded-full
                      border
                      border-[#8BCF63]
                      text-[10px]
                      text-[#8BCF63]
                      transition-all
                      duration-300
                      group-hover:scale-110
                      group-hover:bg-[#8BCF63]
                      group-hover:text-[#061810]
                    "
                  >
                    ✓
                  </span>
                  Farm supplies and tools
                </div>

                {/* Feature 3 */}
                <div
                  className="
                    animate-entrance-up
                    entrance-delay-700
                    group
                    flex
                    items-center
                    gap-2
                    text-xs
                    text-white/65
                    transition-all
                    duration-300
                    hover:-translate-y-1
                    hover:text-white
                  "
                >
                  <span
                    className="
                      flex
                      h-5
                      w-5
                      items-center
                      justify-center
                      rounded-full
                      border
                      border-[#8BCF63]
                      text-[10px]
                      text-[#8BCF63]
                      transition-all
                      duration-300
                      group-hover:scale-110
                      group-hover:bg-[#8BCF63]
                      group-hover:text-[#061810]
                    "
                  >
                    ✓
                  </span>
                  Machinery and services
                </div>
              </div>
            </div>

            {/* ========================================
                RIGHT IMAGE
            ======================================== */}

            <div
              className="
                animate-entrance-right
                entrance-delay-300
                group
                relative
                mx-auto
                w-full
                max-w-md
              "
            >
              {/* Glow */}
              <div
                className="
                  absolute
                  -inset-5
                  rounded-[36px]
                  bg-[#8BCF63]/5
                  opacity-0
                  blur-2xl
                  transition-all
                  duration-700
                  group-hover:opacity-100
                "
              />

              {/* Offset Border */}
              <div
                className="
                  absolute
                  -bottom-2
                  -right-2
                  h-full
                  w-full
                  rounded-[30px]
                  border
                  border-[#8BCF63]/70
                  transition-all
                  duration-500
                  group-hover:translate-x-1
                  group-hover:translate-y-1
                  group-hover:border-[#8BCF63]
                "
              />

              {/* Spinning Circle */}
              <div
                className="
                  animate-entrance-scale
                  entrance-delay-700
                  absolute
                  -right-8
                  -top-8
                  h-20
                  w-20
                  rounded-full
                  border
                  border-dashed
                  border-[#8BCF63]/30
                "
              />

              {/* Image */}
              <div className="relative h-[460px] overflow-hidden rounded-[30px]">
                <img
                  src="https://images.pexels.com/photos/5174197/pexels-photo-5174197.jpeg"
                  alt="E-Agro agricultural field"
                  className="
                    h-full
                    w-full
                    object-cover
                    transition-all
                    duration-1000
                    ease-out
                    group-hover:scale-105
                    group-hover:rotate-[0.5deg]
                  "
                />

                {/* Gradient */}
                <div
                  className="
                    absolute
                    inset-0
                    bg-gradient-to-t
                    from-black/40
                    to-transparent
                    transition-all
                    duration-700
                    group-hover:from-black/20
                  "
                />

                {/* Green Overlay */}
                <div
                  className="
                    absolute
                    inset-0
                    bg-[#8BCF63]/0
                    transition-colors
                    duration-700
                    group-hover:bg-[#8BCF63]/5
                  "
                />
              </div>

              {/* ========================================
                  FLOATING CARD
              ======================================== */}

              <div
                className="
                  animate-entrance-up
                  entrance-delay-700
                  absolute
                  -bottom-2
                  -left-8
                  w-64
                  overflow-hidden
                  rounded-2xl
                  border
                  border-white/10
                  bg-[#10271A]/95
                  shadow-2xl
                  backdrop-blur-md
                  transition-all
                  duration-500
                  hover:-translate-y-1
                  sm:-bottom-5
                "
              >
                {/* Farmer */}
                <div
                  className="
                    group/item
                    flex
                    items-center
                    justify-between
                    border-b
                    border-white/10
                    px-4
                    py-4
                    transition-all
                    duration-300
                    hover:bg-white/5
                  "
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="
                        flex
                        h-8
                        w-8
                        items-center
                        justify-center
                        rounded-full
                        bg-green-500/15
                        text-[#8BCF63]
                        transition-all
                        duration-300
                        group-hover/item:scale-110
                        group-hover/item:rotate-6
                        group-hover/item:bg-green-500/25
                      "
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        aria-hidden="true"
                        width="18"
                        height="18"
                        viewBox="0 0 24 24"
                      >
                        <path
                          fill="none"
                          stroke="currentColor"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M14 9.536V7a4 4 0 0 1 4-4h1.5a.5.5 0 0 1 .5.5V5a4 4 0 0 1-4 4a4 4 0 0 0-4 4c0 2 0 3 1 5a5 5 0 0 1-1 3M4 9a5 5 0 0 1 8 4a5 5 0 0 1-8-4m1 12h14"
                        />
                      </svg>
                    </div>

                    <div>
                      <p className="text-xs font-semibold text-white">
                        I'm a farmer
                      </p>

                      <p className="text-[10px] text-white/40">
                        Loans, supplies, machinery
                      </p>
                    </div>
                  </div>

                  <span
                    className="
                      text-white/40
                      transition-transform
                      duration-300
                      group-hover/item:translate-x-1
                    "
                  >
                    ›
                  </span>
                </div>

                {/* Supplier */}
                <div
                  className="
                    group/item
                    flex
                    items-center
                    justify-between
                    px-4
                    py-4
                    transition-all
                    duration-300
                    hover:bg-white/5
                  "
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="
                        flex
                        h-8
                        w-8
                        items-center
                        justify-center
                        rounded-full
                        bg-yellow-500/10
                        text-[#F5B900]
                        transition-all
                        duration-300
                        group-hover/item:scale-110
                        group-hover/item:rotate-6
                        group-hover/item:bg-yellow-500/20
                      "
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        aria-hidden="true"
                        width="18"
                        height="18"
                        viewBox="0 0 24 24"
                      >
                        <path
                          fill="none"
                          stroke="currentColor"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M21 16.008V7.99a1.98 1.98 0 0 0-1-1.717l-7-4.008a2.02 2.02 0 0 0-2 0L4 6.273c-.619.355-1 1.01-1 1.718v8.018c0 .709.381 1.363 1 1.717l7 4.008a2.02 2.02 0 0 0 2 0l7-4.008c.619-.354 1-1.009-1-1.717M12 22V12m0 0l8.73-5.04m-17.46 0L12 12"
                        />
                      </svg>
                    </div>

                    <div>
                      <p className="text-xs font-semibold text-white">
                        I'm a supplier
                      </p>

                      <p className="text-[10px] text-white/40">
                        Connect with farmers
                      </p>
                    </div>
                  </div>

                  <span
                    className="
                      text-white/40
                      transition-transform
                      duration-300
                      group-hover/item:translate-x-1
                    "
                  >
                    ›
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}

export default App;
